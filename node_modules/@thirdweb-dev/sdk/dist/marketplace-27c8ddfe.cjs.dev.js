...
var marketplaceAuction = require('./marketplace-auction-9024f3da.cjs.dev.js');
...
class Marketplace {
  ...
  constructor(network, address, storage) {
    ...
    let contractWrapper = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : new custom.ContractWrapper(network, address, abi, options);
     ...
     * ```
     */
    QueryParams._defineProperty(this, "makeOffer", custom.buildTransactionFunction(async (listingId, pricePerToken, quantity) => {
      const listing = await this.contractWrapper.readContract.listings(listingId);
      if (listing.listingId.toString() !== listingId.toString()) {
        throw new custom.ListingNotFoundError(this.getAddress(), listingId.toString());
      }
        ...
        case marketplaceAuction.ListingType.Direct:
          {
            ...
            return await this.direct.makeOffer.prepare(listingId, quantity, custom.isNativeToken(listing.currency) ? custom.NATIVE_TOKENS[chainId].wrapped.address : listing.currency, pricePerToken);
          }
        case marketplaceAuction.ListingType.Auction:
          {
     ...
     */
    QueryParams._defineProperty(this, "setBidBufferBps", custom.buildTransactionFunction(async bufferBps => {
     ...
     */
    QueryParams._defineProperty(this, "setTimeBufferInSeconds", custom.buildTransactionFunction(async bufferInSeconds => {
      await this.roles.verify(["admin"], await this.contractWrapper.getSignerAddress());
     ...
     */
    QueryParams._defineProperty(this, "allowListingFromSpecificAssetOnly", custom.buildTransactionFunction(async contractAddress => {
      const encoded = [];
      const members = await this.roles.get("asset");
      if (members.includes(ethers.constants.AddressZero)) {
        encoded.push(this.encoder.encode("revokeRole", [custom.getRoleHash("asset"), ethers.constants.AddressZero]));
      }
      encoded.push(this.encoder.encode("grantRole", [custom.getRoleHash("asset"), contractAddress]));
      return custom.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "multicall",
        args: [encoded]
     ...
     */
    QueryParams._defineProperty(this, "allowListingFromAnyAsset", custom.buildTransactionFunction(async () => {
      const encoded = [];
      const members = await this.roles.get("asset");
      for (const addr in members) {
        encoded.push(this.encoder.encode("revokeRole", [custom.getRoleHash("asset"), addr]));
      }
      encoded.push(this.encoder.encode("grantRole", [custom.getRoleHash("asset"), ethers.constants.AddressZero]));
      return custom.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "multicall",
        args: [encoded]
    ...
    this.metadata = new custom.ContractMetadata(this.contractWrapper, custom.MarketplaceContractSchema, this.storage);
    this.app = new custom.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new custom.ContractRoles(this.contractWrapper, Marketplace.contractRoles);
    ...
    this.platformFees = new custom.ContractPlatformFee(this.contractWrapper);
  ...
  async isRestrictedToListerRoleOnly() {
    const anyoneCanList = await this.contractWrapper.readContract.hasRole(custom.getRoleHash("lister"), ethers.constants.AddressZero);
  ...
  async getOffers(listingId) {
    ...
    return await Promise.all(listingEvents.map(async e => {
      return await custom.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), {
  ...
  async getAllListingsNoFilter(filterInvalidListings) {
    const listings = await Promise.all(Array.from(Array((await this.contractWrapper.readContract.totalListings()).toNumber()).keys()).map(async i => {
      ...
      try {
        ...
        if (err instanceof custom.ListingNotFoundError) {
          return undefined;
        } else {
