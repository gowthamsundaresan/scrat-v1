...
var custom = require('./custom-1071eb36.cjs.dev.js');
...
class MarketplaceDirect {
  constructor(contractWrapper, storage) {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "createListing", custom.buildTransactionFunction(async listing => {
      custom.validateNewListingParam(listing);
      const resolvedAssetAddress = await custom.resolveAddress(listing.assetContractAddress);
      const resolvedCurrencyAddress = await custom.resolveAddress(listing.currencyContractAddress);
      await custom.handleTokenApproval(this.contractWrapper, this.getAddress(), resolvedAssetAddress, listing.tokenId, await this.contractWrapper.getSignerAddress());
      const normalizedPricePerToken = await custom.normalizePriceValue(this.contractWrapper.getProvider(), listing.buyoutPricePerToken, resolvedCurrencyAddress);
      ...
      return custom.Transaction.fromContractWrapper({
        ...
        args: [{
          ...
          currencyToAccept: custom.cleanCurrencyAddress(resolvedCurrencyAddress),
     ...
     * ```
     */
    QueryParams._defineProperty(this, "createListingsBatch", custom.buildTransactionFunction(async listings => {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "makeOffer", custom.buildTransactionFunction(async (listingId, quantityDesired, currencyContractAddress, pricePerToken, expirationDate) => {
      if (custom.isNativeToken(currencyContractAddress)) {
        throw new Error("You must use the wrapped native token address when making an offer with a native token");
      }
      const normalizedPrice = await custom.normalizePriceValue(this.contractWrapper.getProvider(), pricePerToken, currencyContractAddress);
      try {
      ...
      await custom.setErc20Allowance(this.contractWrapper, value, currencyContractAddress, overrides);
     ...
     * ```
     */
    QueryParams._defineProperty(this, "acceptOffer", custom.buildTransactionFunction(async (listingId, addressOfOfferor) => {
      /**
       * TODO:
       * - Provide better error handling if offer is too low.
       */
      await this.validateListing(ethers.BigNumber.from(listingId));
      const resolvedAddress = await custom.resolveAddress(addressOfOfferor);
     ...
     */
    QueryParams._defineProperty(this, "buyoutListing", custom.buildTransactionFunction(async (listingId, quantityDesired, receiver) => {
      ...
      const overrides = (await this.contractWrapper.getCallOverrides()) || {};
      await custom.setErc20Allowance(this.contractWrapper, value, listing.currencyContractAddress, overrides);
      return custom.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "buy",
     ...
     */
    QueryParams._defineProperty(this, "updateListing", custom.buildTransactionFunction(async listing => {
      return custom.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "updateListing",
        args: [listing.id, listing.quantity, listing.buyoutPrice,
        // reserve price, doesn't matter for direct listing
        listing.buyoutPrice, await custom.resolveAddress(listing.currencyContractAddress), listing.startTimeInSeconds, listing.secondsUntilEnd]
      });
    }));
    /**
     ...
     * ```
     */
    QueryParams._defineProperty(this, "cancelListing", custom.buildTransactionFunction(async listingId => {
      return custom.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
  ...
  async getListing(listingId) {
    ...
    if (listing.assetContract === ethers.constants.AddressZero) {
      throw new custom.ListingNotFoundError(this.getAddress(), listingId.toString());
    }
    if (listing.listingType !== ListingType.Direct) {
      throw new custom.WrongListingTypeError(this.getAddress(), listingId.toString(), "Auction", "Direct");
    }
  ...
  async getActiveOffer(listingId, address) {
    ...
    const offers = await this.contractWrapper.readContract.offers(listingId, await custom.resolveAddress(address));
    if (offers.offeror === ethers.constants.AddressZero) {
      return undefined;
    }
    return await custom.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), offers);
  }
  /** ******************************
   * PRIVATE FUNCTIONS
   *******************************/
  ...
  async mapListing(listing) {
    return {
      ...
      buyoutCurrencyValuePerToken: await custom.fetchCurrencyValue(this.contractWrapper.getProvider(), listing.currency, listing.buyoutPricePerToken),
      id: listing.listingId.toString(),
      tokenId: listing.tokenId,
      quantity: listing.quantity,
      startTimeInSeconds: listing.startTime,
      asset: await custom.fetchTokenMetadataForContract(listing.assetContract, this.contractWrapper.getProvider(), listing.tokenId, this.storage),
  ...
  async isStillValidListing(listing, quantity) {
    const approved = await custom.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), listing.assetContractAddress, listing.tokenId, listing.sellerAddress);
    if (!approved) {
      return {
        valid: false,
    ...
    const isERC721 = await erc165.supportsInterface(custom.InterfaceId_IERC721);
    const isERC1155 = await erc165.supportsInterface(custom.InterfaceId_IERC1155);
    if (isERC721) {
...
class MarketplaceAuction {
  constructor(contractWrapper, storage) {
    ...
    QueryParams._defineProperty(this, "createListing", custom.buildTransactionFunction(async listing => {
      ...
      await custom.handleTokenApproval(this.contractWrapper, this.getAddress(), resolvedAssetAddress, listing.tokenId, await this.contractWrapper.getSignerAddress());
      const normalizedPricePerToken = await custom.normalizePriceValue(this.contractWrapper.getProvider(), listing.buyoutPricePerToken, resolvedCurrencyAddress);
      const normalizedReservePrice = await custom.normalizePriceValue(this.contractWrapper.getProvider(), listing.reservePricePerToken, resolvedCurrencyAddress);
      let listingStartTime = Math.floor(listing.startTimestamp.getTime() / 1000);
      const block = await this.contractWrapper.getProvider().getBlock("latest");
      const blockTime = block.timestamp;
      if (listingStartTime < blockTime) {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "buyoutListing", custom.buildTransactionFunction(async listingId => {
      const listing = await this.validateListing(ethers.BigNumber.from(listingId));
      const currencyMetadata = await custom.fetchCurrencyMetadata(this.contractWrapper.getProvider(), listing.currencyContractAddress);
     ...
     * ```
     */
    QueryParams._defineProperty(this, "makeBid", custom.buildTransactionFunction(async (listingId, pricePerToken) => {
      const listing = await this.validateListing(ethers.BigNumber.from(listingId));
      const normalizedPrice = await custom.normalizePriceValue(this.contractWrapper.getProvider(), pricePerToken, listing.currencyContractAddress);
      ...
      if (winningBid) {
        const isWinner = custom.isWinningBid(winningBid.pricePerToken, normalizedPrice, bidBuffer);
    ...
    QueryParams._defineProperty(this, "cancelListing", custom.buildTransactionFunction(async listingId => {
      ...
      if (now.gt(startTime) && offers.offeror !== ethers.constants.AddressZero) {
        throw new custom.AuctionAlreadyStartedError(listingId.toString());
      }
      return custom.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "closeAuction",
     ...
     */
    QueryParams._defineProperty(this, "closeListing", custom.buildTransactionFunction(async (listingId, closeFor) => {
      if (!closeFor) {
      ...
      try {
        ...
        if (err.message.includes("cannot close auction before it has ended")) {
          throw new custom.AuctionHasNotEndedError(listingId.toString(), listing.endTimeInEpochSeconds.toString());
        } else {
          throw err;
        }
      }
     ...
     * @param listingId - the auction  listing ud to close
     */
    QueryParams._defineProperty(this, "executeSale", custom.buildTransactionFunction(async listingId => {
      const listing = await this.validateListing(ethers.BigNumber.from(listingId));
      try {
        const winningBid = await this.getWinningBid(listingId);
    ...
    this.contractWrapper = contractWrapper;
    this.storage = storage;
    this.encoder = new custom.ContractEncoder(contractWrapper);
  }
  getAddress() {
    return this.contractWrapper.readContract.address;
  }
  ...
  async getListing(listingId) {
    ...
    if (listing.listingType !== ListingType.Auction) {
      throw new custom.WrongListingTypeError(this.getAddress(), listingId.toString(), "Direct", "Auction");
    }
    return await this.mapListing(listing);
  }
  ...
  async getMinimumNextBid(listingId) {
    ...
    return custom.fetchCurrencyValue(this.contractWrapper.getProvider(), listing.currencyContractAddress, minimumNextBid);
  }
  ...
  async mapListing(listing) {
    return {
      ...
      asset: await custom.fetchTokenMetadataForContract(listing.assetContract, this.contractWrapper.getProvider(), listing.tokenId, this.storage),
      reservePriceCurrencyValuePerToken: await custom.fetchCurrencyValue(this.contractWrapper.getProvider(), listing.currency, listing.reservePricePerToken),
