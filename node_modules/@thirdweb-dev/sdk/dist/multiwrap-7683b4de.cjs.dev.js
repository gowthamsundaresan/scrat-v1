...
class Multiwrap extends erc721Standard.StandardErc721 {
  ...
  constructor(network, address, storage) {
     ...
     */
    QueryParams._defineProperty(this, "wrap", custom.buildTransactionFunction(async (contents, wrappedTokenMetadata, recipientAddress) => {
      const uri = await custom.uploadOrExtractURI(wrappedTokenMetadata, this.storage);
      const recipient = await custom.resolveAddress(recipientAddress ? recipientAddress : await this.contractWrapper.getSignerAddress());
     ...
     */
    QueryParams._defineProperty(this, "unwrap", custom.buildTransactionFunction(async (wrappedTokenId, recipientAddress) => {
      const recipient = await custom.resolveAddress(recipientAddress ? recipientAddress : await this.contractWrapper.getSignerAddress());
      return custom.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "unwrap",
    ...
    this.metadata = new custom.ContractMetadata(this.contractWrapper, custom.MultiwrapContractSchema, this.storage);
    this.app = new custom.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new custom.ContractRoles(this.contractWrapper, Multiwrap.contractRoles);
  ...
  async getWrappedContents(wrappedTokenId) {
    ...
    for (const token of wrappedTokens) {
      switch (token.tokenType) {
        case 0:
          {
            const tokenMetadata = await custom.fetchCurrencyMetadata(this.contractWrapper.getProvider(), token.assetContract);
            erc20Tokens.push({
  ...
  async toTokenStructList(contents) {
    ...
    if (contents.erc20Tokens) {
      for (const erc20 of contents.erc20Tokens) {
        const normalizedQuantity = await custom.normalizePriceValue(provider, erc20.quantity, erc20.contractAddress);
        const hasAllowance = await custom.hasERC20Allowance(this.contractWrapper, erc20.contractAddress, normalizedQuantity);
        if (!hasAllowance) {
