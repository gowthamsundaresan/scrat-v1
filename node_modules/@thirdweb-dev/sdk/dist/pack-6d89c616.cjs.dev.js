...
var erc1155Standard = require('./erc-1155-standard-38cc5de8.cjs.dev.js');
...
const CommonWrappableSchema = zod.z.object({
  contractAddress: custom.AddressOrEnsSchema
});
...
const ERC721WrappableSchema = CommonWrappableSchema.extend({
  tokenId: custom.BigNumberishSchema
});
...
const ERC1155WrappableSchema = CommonWrappableSchema.extend({
  tokenId: custom.BigNumberishSchema,
  quantity: custom.BigNumberishSchema
});
...
const ERC1155RewardSchema = ERC1155WrappableSchema.omit({
  ...
  quantityPerReward: custom.BigNumberishSchema
});
...
const ERC20RewardContentsSchema = ERC20RewardSchema.extend({
  totalRewards: custom.BigNumberishSchema.default("1")
});
...
const PackMetadataInputSchema = PackRewardsOutputSchema.extend({
  ...
  rewardsPerPack: custom.BigNumberishSchema.default("1"),
  openStartTime: custom.RawDateSchema.default(new Date())
});
...
class PackVRF {
  constructor(network, address, storage, options, chainId) {
    var _this = this;
    let contractWrapper = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : new custom.ContractWrapper(network, address, IPackVRFAbi__default["default"], options);
    QueryParams._defineProperty(this, "featureName", custom.FEATURE_PACK_VRF.name);
     ...
     * @twfeature PackVRF
     */
    QueryParams._defineProperty(this, "open", custom.buildTransactionFunction(async function (tokenId) {
     ...
     * @twfeature PackVRF
     */
    QueryParams._defineProperty(this, "claimRewards", custom.buildTransactionFunction(async function () {
    ...
    this.events = new custom.ContractEvents(this.contractWrapper);
  }
  ...
  async parseRewards(rewards) {
    ...
    for (const reward of rewards) {
      switch (reward.tokenType) {
        case 0:
          {
            const tokenMetadata = await custom.fetchCurrencyMetadata(this.contractWrapper.getProvider(), reward.assetContract);
            erc20Rewards.push({
  ...
  async canClaimRewards(claimerAddress) {
    const address = await custom.resolveAddress(claimerAddress || (await this.contractWrapper.getSignerAddress()));
  ...
  getLinkContract() {
    const linkAddress = custom.LINK_TOKEN_ADDRESS[this.chainId];
    if (!linkAddress) {
      throw new Error(`No LINK token address found for chainId ${this.chainId}`);
    }
    const contract = new custom.ContractWrapper(this.contractWrapper.getSignerOrProvider(), linkAddress, ERC20Abi__default["default"], this.contractWrapper.options);
    return new custom.Erc20(contract, this.storage, this.chainId);
  }
}
...
class Pack extends erc1155Standard.StandardErc1155 {
   ...
   */
  get vrf() {
    return custom.assertEnabled(this._vrf, custom.FEATURE_PACK_VRF);
  }
  constructor(network, address, storage) {
    ...
    let contractWrapper = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : new custom.ContractWrapper(network, address, abi, options.gasless && "openzeppelin" in options.gasless ? {
      ...options,
      gasless: {
        ...options.gasless,
        openzeppelin: {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "create", custom.buildTransactionFunction(async metadataWithRewards => {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "addPackContents", custom.buildTransactionFunction(async (packId, packContents) => {
      const signerAddress = await this.contractWrapper.getSignerAddress();
     ...
     * ```
     */
    QueryParams._defineProperty(this, "createTo", custom.buildTransactionFunction(async (to, metadataWithRewards) => {
      const uri = await custom.uploadOrExtractURI(metadataWithRewards.packMetadata, this.storage);
      ...
      return custom.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "createPack",
        args: [contents, numOfRewardUnits, uri, parsedMetadata.openStartTime, parsedMetadata.rewardsPerPack, await custom.resolveAddress(to)],
        parse: receipt => {
        ...
        parse: async receipt => {
          ...
          for (const reward of rewards) {
            switch (reward.tokenType) {
              case 0:
                {
                  const tokenMetadata = await custom.fetchCurrencyMetadata(_this.contractWrapper.getProvider(), reward.assetContract);
                  erc20Rewards.push({
                    contractAddress: reward.assetContract,
                    quantityPerReward: ethers.ethers.utils.formatUnits(reward.totalAmount, tokenMetadata.decimals).toString()
                  });
    ...
    this.abi = custom.AbiSchema.parse(abi || []);
    this.metadata = new custom.ContractMetadata(this.contractWrapper, custom.PackContractSchema, this.storage);
    this.app = new custom.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new custom.ContractRoles(this.contractWrapper, Pack.contractRoles);
    this.royalties = new custom.ContractRoyalty(this.contractWrapper, this.metadata);
    this.encoder = new custom.ContractEncoder(this.contractWrapper);
    this.estimator = new custom.GasCostEstimator(this.contractWrapper);
    this.events = new custom.ContractEvents(this.contractWrapper);
    this.interceptor = new custom.ContractInterceptor(this.contractWrapper);
    this.owner = new custom.ContractOwner(this.contractWrapper);
  ...
  async isTransferRestricted() {
    const anyoneCanTransfer = await this.contractWrapper.readContract.hasRole(custom.getRoleHash("transfer"), ethers.ethers.constants.AddressZero);
  ...
  async toPackContentArgs(metadataWithRewards) {
    ...
    for (const erc20 of erc20Rewards) {
      const normalizedQuantity = await custom.normalizePriceValue(provider, erc20.quantityPerReward, erc20.contractAddress);
      ...
      const hasAllowance = await custom.hasERC20Allowance(this.contractWrapper, erc20.contractAddress, totalQuantity);
      if (!hasAllowance) {
    ...
    for (const erc721 of erc721Rewards) {
      const isApproved = await custom.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc721.contractAddress, erc721.tokenId, owner);
      if (!isApproved) {
    ...
    for (const erc1155 of erc1155Rewards) {
      const isApproved = await custom.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc1155.contractAddress, erc1155.tokenId, owner);
      if (!isApproved) {
  ...
  detectVrf() {
    if (custom.detectContractFeature(this.contractWrapper, "PackVRF")) {
