...
class Split {
  ...
  constructor(network, address, storage) {
     ...
     */
    QueryParams._defineProperty(this, "withdraw", custom.buildTransactionFunction(async walletAddress => {
      return custom.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "release(address)",
        args: [await custom.resolveAddress(walletAddress)]
      });
    }));
    /**
     ...
     */
    QueryParams._defineProperty(this, "withdrawToken", custom.buildTransactionFunction(async (walletAddress, tokenAddress) => {
      return custom.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "release(address,address)",
        args: [await custom.resolveAddress(tokenAddress), await custom.resolveAddress(walletAddress)]
      });
    }));
    /**
     * Distribute Funds
     ...
     * ```
     */
    QueryParams._defineProperty(this, "distribute", custom.buildTransactionFunction(async () => {
      return custom.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "distribute()",
        args: []
     ...
     * @param tokenAddress - The address of the currency contract to distribute funds
     */
    QueryParams._defineProperty(this, "distributeToken", custom.buildTransactionFunction(async tokenAddress => {
      return custom.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "distribute(address)",
        args: [await custom.resolveAddress(tokenAddress)]
      });
    }));
    ...
    this.metadata = new custom.ContractMetadata(this.contractWrapper, custom.SplitsContractSchema, this.storage);
    this.app = new custom.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new custom.ContractRoles(this.contractWrapper, Split.contractRoles);
  ...
  async balanceOfTokenAllRecipients(tokenAddress) {
    const resolvedToken = await custom.resolveAddress(tokenAddress);
    const recipients = await this.getAllRecipients();
    const balances = {};
    for (const recipient of recipients) {
  ...
  async balanceOf(address) {
    const resolvedAddress = await custom.resolveAddress(address);
  ...
  async balanceOfToken(walletAddress, tokenAddress) {
    const resolvedToken = await custom.resolveAddress(tokenAddress);
    const resolvedWallet = await custom.resolveAddress(walletAddress);
    ...
    return await custom.fetchCurrencyValue(this.contractWrapper.getProvider(), resolvedToken, value);
  }
  ...
  async _pendingPayment(address, totalReceived, alreadyReleased) {
    const addressReceived = totalReceived.mul(await this.contractWrapper.readContract.shares(await custom.resolveAddress(address)));
