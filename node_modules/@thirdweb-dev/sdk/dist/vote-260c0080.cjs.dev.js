...
class Vote {
  ...
  constructor(network, address, storage) {
     ...
     */
    QueryParams._defineProperty(this, "propose", custom.buildTransactionFunction(async (description, executions) => {
      if (!executions) {
        executions = [{
     ...
     */
    QueryParams._defineProperty(this, "vote", custom.buildTransactionFunction(async function (proposalId, voteType) {
     ...
     */
    QueryParams._defineProperty(this, "execute", custom.buildTransactionFunction(async proposalId => {
    ...
    this.metadata = new custom.ContractMetadata(this.contractWrapper, custom.VoteContractSchema, this.storage);
  ...
  async hasVoted(proposalId, account) {
    ...
    return this.contractWrapper.readContract.hasVoted(proposalId, await custom.resolveAddress(account));
  }
  ...
  async balanceOfToken(tokenAddress) {
    const erc20 = new ethers.Contract(await custom.resolveAddress(tokenAddress), ERC20Abi__default["default"], this.contractWrapper.getProvider());
    return await custom.fetchCurrencyValue(this.contractWrapper.getProvider(), tokenAddress, await erc20.balanceOf(this.contractWrapper.readContract.address));
  }
  ...
  async settings() {
    ...
    const votingTokenMetadata = await custom.fetchCurrencyMetadata(this.contractWrapper.getProvider(), votingTokenAddress);
    return {
