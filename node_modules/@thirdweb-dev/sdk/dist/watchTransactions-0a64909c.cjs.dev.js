...
async function getContractFromAbi(params) {
  const resolvedAddress = await custom.resolveAddress(params.address);
  const [signer, provider] = custom.getSignerAndProvider(params.network, params.sdkOptions);
  ...
  const contract = new custom.SmartContract(signer || provider, resolvedAddress, await custom.getCompositePluginABI(resolvedAddress, custom.AbiSchema.parse(parsedAbi), provider, params.sdkOptions, getCachedStorage(params.storage)), getCachedStorage(params.storage), params.sdkOptions, chainId);
...
async function resolveContractType(params) {
  try {
    ...
    .replace(/\x00/g, "");
    return custom.getContractTypeForRemoteName(remoteContractType);
  } catch (err) {
    return "custom";
  }
}
...
async function getContract(params) {
  ...
  if (!params.contractTypeOrAbi || params.contractTypeOrAbi === "custom") {
    ...
    if (contractType === "custom") {
      const publisher = new custom.ContractPublisher(params.network, params.sdkOptions, getCachedStorage(params.storage));
      try {
      ...
      const abi = await custom.PREBUILT_CONTRACTS_MAP[contractType].getAbi(resolvedAddress, provider, getCachedStorage(params.storage));
      return getContractFromAbi({
        ...params,
        address: resolvedAddress,
        abi
  ...
  } else if (typeof params.contractTypeOrAbi === "string" && params.contractTypeOrAbi in custom.PREBUILT_CONTRACTS_MAP) {
    const contract = await custom.PREBUILT_CONTRACTS_MAP[params.contractTypeOrAbi].initialize(signer || provider, resolvedAddress, getCachedStorage(params.storage), params.sdkOptions);
    cacheContract(contract, resolvedAddress, chainId);
    return contract;
  } else {
    return getContractFromAbi({
...
async function getChainId(params) {
  const [, provider] = custom.getSignerAndProvider(params.network, params.sdkOptions);
  let chainIdPromise;
