...
async function zkDeployContractFromUri(publishMetadataUri, constructorParamValues, signer, storage, chainId, options) {
  const {
    compilerMetadata,
    extendedMetadata
  } = await custom.fetchAndCacheDeployMetadata(publishMetadataUri, storage);
  ...
  if (extendedMetadata && extendedMetadata.factoryDeploymentData && (extendedMetadata.isDeployableViaProxy || extendedMetadata.isDeployableViaFactory) && !forceDirectDeploy) {
    ...
    const initializerParamTypes = custom.extractFunctionParamsFromAbi(compilerMetadata.abi, extendedMetadata.factoryDeploymentData.implementationInitializerFunction).map(p => p.type);
    const paramValues = custom.convertParamValues(initializerParamTypes, constructorParamValues);
...
async function registerContractOnMultiChainRegistry(address, chainId, metadataURI) {
  try {
    ...
    const sdk = custom.ThirdwebSDK.fromPrivateKey(wallet.privateKey, "polygon", {
      gasless: {
        openzeppelin: {
